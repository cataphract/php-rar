...
int PASCAL RARReadHeaderEx(HANDLE hArcData,struct RARHeaderDataEx *D)
{
  ...
  try
  {
    ...
    D->CmtSize=0;
    D->CmtState=0;
	/* these three next lines were added by me */
	_copyTime(Data->Arc.FileHead.mtime, (RarLocalTime *) &D->mtime);
	_copyTime(Data->Arc.FileHead.ctime, (RarLocalTime *) &D->ctime);
	_copyTime(Data->Arc.FileHead.atime, (RarLocalTime *) &D->atime);
